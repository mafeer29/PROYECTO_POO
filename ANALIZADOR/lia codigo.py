...
class PoblacionActiva:
    def __init__(self, excel_path_poblacion, excel_path_estaciones=None):
        self.excel_path_poblacion = excel_path_poblacion
        self.excel_path_estaciones = excel_path_estaciones
        self.df = None
        self.df_filtered = None
        self.df_total = None
        self.df_reducido = None
        self.df_estaciones = None
        self.df_estaciones_grouped = None

    def cargar_datos_poblacion(self, sheet_name='PET1'):
        """Carga los datos desde el archivo Excel de población activa"""
        self.df = pd.read_excel(self.excel_path_poblacion, sheet_name=sheet_name, header=0)

        # Renombrar la columna si la primera aparece como NaN
        if pd.isna(self.df.columns[0]):
            self.df.rename(columns={self.df.columns[0]: 'Departamento, provincia, área urbana y rural; y sexo'}, inplace=True)

        # Limpiar nombres de columnas
        self.df.columns = self.df.columns.str.strip().str.replace('\n', '')

    def extraer_provincias(self):
        """Extrae las provincias y asigna valores correspondientes"""
        self.df['PROVINCIA'] = None
        provincia_actual = None
        for index, row in self.df.iterrows():
            columna_principal = row['Departamento, provincia, área urbana y rural; y sexo']
            if isinstance(columna_principal, str):
                ...
                if columna_principal.isupper() and "URBANA" not in columna_principal and "RURAL" not in columna_principal:
                    provincia_actual = columna_principal
                else:
                    self.df.at[index, 'PROVINCIA'] = provincia_actual

    def filtrar_urbana_rural(self):
        """Filtra las filas que contienen 'URBANA' o 'RURAL' en la columna principal"""
        self.df_filtered = self.df[self.df['Departamento, provincia, área urbana y rural; y sexo'].str.contains('URBANA|RURAL', na=False)]

    def obtener_poblacion_total(self):
        """Obtiene el DataFrame de la población total"""
        self.df_total = self.df_filtered[['PROVINCIA', 'Departamento, provincia, área urbana y rural; y sexo',
                                          'Total', '14 a 29', '30 a 44', '45 a 64', '65 y más']]

        self.df_total['DEPARTAMENTO'] = self.df_total['PROVINCIA'].apply(lambda x: x.split()[0] if isinstance(x, str) else None)
        self.df_total = self.df_total[['DEPARTAMENTO', 'PROVINCIA', 'Departamento, provincia, área urbana y rural; y sexo',
                                       'Total', '14 a 29', '30 a 44', '45 a 64', '65 y más']]

    def ajustar_poblacion(self, row):
        """Ajusta los valores de la columna '65 y más' según área urbana o rural"""
        if 'URBANA' in row['Departamento, provincia, área urbana y rural; y sexo']:
            return round(row['65 y más'] * 0.35)
        elif 'RURAL' in row['Departamento, provincia, área urbana y rural; y sexo']:
            return round(row['65 y más'] * 0.20)
        return row['65 y más']

    def obtener_poblacion_reducida(self):
        """Obtiene el DataFrame con los datos de población reducida"""
        self.df_reducido = self.df_filtered.copy()
        self.df_reducido['65 y más'] = self.df_reducido.apply(self.ajustar_poblacion, axis=1).astype(int)
        self.df_reducido['Total'] = self.df_reducido[['14 a 29', '30 a 44', '45 a 64', '65 y más']].sum(axis=1).astype(int)
        self.df_reducido['DEPARTAMENTO'] = self.df_reducido['PROVINCIA'].apply(lambda x: x.split()[0] if isinstance(x, str) else None)
        self.df_reducido = self.df_reducido[['DEPARTAMENTO', 'PROVINCIA', 'Departamento, provincia, área urbana y rural; y sexo',
                                             'Total', '14 a 29', '30 a 44', '45 a 64', '65 y más']]

    def guardar_resultados(self):
        """Guarda los DataFrames generados en archivos Excel"""
    ...
    def cargar_datos_estaciones(self):
        """Carga el archivo de estaciones base si se proporciona"""
        if self.excel_path_estaciones:
            self.df_estaciones = pd.read_excel(self.excel_path_estaciones, header=0)

            # Limpiar nombres de columnas
            self.df_estaciones.columns = self.df_estaciones.columns.str.strip()

    def agrupar_estaciones_por_provincia(self):
        """Agrupa las estaciones base por DEPARTAMENTO y PROVINCIA y calcula el total"""
        if self.df_estaciones is not None:
            self.df_estaciones_grouped = self.df_estaciones.groupby(['DEPARTAMENTO', 'PROVINCIA'])[['CANT_EB_2G', 'CANT_EB_3G', 'CANT_EB_4G', 'CANT_EB_5G']].sum().reset_index()

            # Crear una nueva columna 'TOTAL_ESTACIONES_BASE' sumando las estaciones base de 2G, 3G, 4G y 5G
            self.df_estaciones_grouped['TOTAL_ESTACIONES_BASE'] = self.df_estaciones_grouped[['CANT_EB_2G', 'CANT_EB_3G', 'CANT_EB_4G', 'CANT_EB_5G']].sum(axis=1)

            # Ordenar los datos primero por DEPARTAMENTO y luego por PROVINCIA alfabéticamente
            self.df_estaciones_grouped = self.df_estaciones_grouped.sort_values(by=['DEPARTAMENTO', 'PROVINCIA'], ascending=[True, True]).reset_index(drop=True)

            # Guardar el DataFrame resultante en un nuevo archivo Excel
            self.df_estaciones_grouped.to_excel('estaciones_por_provincia_ordenado_con_total.xlsx', index=False)

    def procesar(self):
        """Método principal que llama a los pasos necesarios"""
        self.cargar_datos_poblacion()
        self.extraer_provincias()
        self.filtrar_urbana_rural()
        self.obtener_poblacion_total()
        self.obtener_poblacion_reducida()
        self.guardar_resultados()

        if self.excel_path_estaciones:
            self.cargar_datos_estaciones()
            self.agrupar_estaciones_por_provincia()
...
if __name__ == "__main__":
    excel_path_poblacion = 'Cuadros Estadístico del Tomo II.xlsx'  # Ruta del archivo Excel de población activa
    excel_path_estaciones = 'COBERTURA_MOVIL_CUANTIFICADA.xlsx'  # Ruta del archivo Excel de estaciones base

    poblacion = PoblacionActiva(excel_path_poblacion, excel_path_estaciones)  # Crear una instancia de la clase
    poblacion.procesar()  # Ejecutar el procesamiento de los datos

    print("Población total:")
    print(poblacion.df_total.head())

    print("\nPoblación reducida:")
    print(poblacion.df_reducido.head())

    print("\nEstaciones base por provincia:")
    print(poblacion.df_estaciones_grouped.head())