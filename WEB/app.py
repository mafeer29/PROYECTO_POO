...
def resultado_centro_poblado():
    ...
    # Crear un archivo CSV en memoria
    output = io.BytesIO()
    info.to_csv(output, index=False, encoding='utf-8')
    output.seek(0)  # Volver al inicio del archivo en memoria
...
# Ruta para descargar el reporte del centro poblado dentro de un distrito específico
@app.route('/descargar_reporte_centro_poblado/<distrito>/<centro_poblado>')
def descargar_reporte_centro_poblado(distrito, centro_poblado):
    # Limpieza de los nombres
    distrito_limpio = distrito.strip()
    centro_poblado_limpio = centro_poblado.strip()
   
    # Filtrar los datos por distrito y centro poblado
    info = df_4[
        (df_4['DISTRITO'] == distrito_limpio) &
        (df_4['CENTRO_POBLADO'] == centro_poblado_limpio)
    ]
    info = info[['DEPARTAMENTO', 'PROVINCIA', 'DISTRITO', 'CENTRO_POBLADO', 
                        'CANT_EB_2G', 'CANT_EB_3G', 'CANT_EB_4G', 'CANT_EB_5G', 'CALIFICACION', 
                        'CANT_EB_TOTAL', 'POBLACION', 'ALCANCE_EB', 'POBLACION_CUBIERTA', 
                        'POBLACION_NO_CUBIERTA', 'EB_NECESARIAS', 'EB_FALTANTES', 'PROPUESTA_EB']]
    ...
    output = io.BytesIO()
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        info.to_excel(writer, index=False, sheet_name='Reporte')
