...
def resultado_provincia():
    ...
    # Datos de la población cubierta vs no cubierta
    poblacion_data = pd.DataFrame({
        'Categoria': ['Población Cubierta', 'Población No Cubierta'],
        'Valor': [df_filtrado['POBLACION_CUBIERTA'].sum(), df_filtrado['POBLACION_NO_CUBIERTA'].sum()]
    })

    # Datos de estaciones base por tecnología
    eb_data = pd.DataFrame({
        'Categoria': ["2G", "3G", "4G", "5G"],
        'Valor': [
            df_filtrado['CANT_EB_2G'].iloc[0], 
            df_filtrado['CANT_EB_3G'].iloc[0], 
            df_filtrado['CANT_EB_4G'].iloc[0], 
            df_filtrado['CANT_EB_5G'].iloc[0]
        ]
    })
    ...
    fig1 = px.bar(df_filtrado, 
                 x='PROVINCIA', 
                 y=['CANT_EB_2G', 'CANT_EB_3G', 'CANT_EB_4G', 'CANT_EB_5G'],
                 title=f'Estaciones Base por Tecnología en {provincia_seleccionada}, {departamento_seleccionado}',
                 labels={'value': 'Cantidad de Estaciones Base', 'variable': 'Tecnología'},
                 barmode='group')
    ...
    # Gráfico circular para comparación de población cubierta vs no cubierta
    fig3 = px.bar(poblacion_data, 
              x='Categoria', 
              y='Valor',
    ...
    fig4 = px.bar(info, 
                  x='PROVINCIA', 
                  y=['EB_NECESARIAS', 'EB_FALTANTES'], 
                  title=f'Comparación de Estaciones Base Necesarias vs Faltantes en {provincia_seleccionada}',
                  labels={'value': 'Cantidad de Estaciones Base', 'variable': 'Tipo'},
                  barmode='group')
...
def resultado_distrito():
    ...
    if df_filtrado.empty:
        return render_template('error.html', message="No se encontraron datos para el distrito seleccionado.")
    ...
    # Extraer las columnas que necesitas mostrar
    info = df_filtrado[['DEPARTAMENTO', 'PROVINCIA', "DISTRITO", 'CANT_EB_2G', 'CANT_EB_3G', 'CANT_EB_4G', 'CANT_EB_5G',
                        'CALIFICACION', 'CANT_EB_TOTAL', 'POBLACION_TOTAL', 'ALCANCE_EB', 'POBLACION_CUBIERTA', 
                        'POBLACION_NO_CUBIERTA', 'EB_NECESARIAS', 'EB_FALTANTES', 'PROPUESTA_EB']]
    propuesta = info["PROPUESTA_EB"].iloc[0]
...
def resultado_centro_poblado():
    ...
    # Verificar el conteo de estaciones base y corregir si es necesario
    # Sumamos las estaciones base de cada tipo de tecnología (si es relevante, asegúrate de que no haya duplicados en la misma estación)
    estaciones_base = df_filtrado[['CANT_EB_2G', 'CANT_EB_3G', 'CANT_EB_4G', 'CANT_EB_5G']].sum()
    
    # Asegurarse de que no haya valores incorrectos (como duplicados) o nulos
    estaciones_base = estaciones_base[estaciones_base > 0]
    
    # Calcular la población cubierta y no cubierta directamente desde el df_filtrado
    poblacion_cubierta = df_filtrado['POBLACION_CUBIERTA'].sum()
    poblacion_no_cubierta = df_filtrado['POBLACION_NO_CUBIERTA'].sum()

    # Verifica los valores de población cubierta y no cubierta
    print("Población Cubierta:", poblacion_cubierta)
    print("Población No Cubierta:", poblacion_no_cubierta)
    
    # Datos de estaciones base por tecnología (se suman las estaciones base para cada tecnología)
    eb_data = pd.DataFrame({
        'Categoria': ["2G", "3G", "4G", "5G"],
        'Valor': [
            df_filtrado['CANT_EB_2G'].sum(), 
            df_filtrado['CANT_EB_3G'].sum(), 
            df_filtrado['CANT_EB_4G'].sum(), 
            df_filtrado['CANT_EB_5G'].sum()
        ]
    })
    ...
    # Gráfico de barras para la comparación de Población Cubierta vs. No Cubierta
    fig3 = px.bar(
        ...
        color=['Población Cubierta', 'Población No Cubierta'],  # Colorear por categorías
        color_discrete_map={'Población Cubierta': '#6379F2', 'Población No Cubierta': '#F24C3D'}  # Colores personalizados
    )
